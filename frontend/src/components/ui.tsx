...
export function Button({
  ...
  const base = "inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed";
  const variants: Record<string, string> = {
    ...
    secondary: "bg-white border border-slate-200 text-slate-700 hover:border-brand-300 hover:text-brand-700",
...
export function EditButton({
  ...
  return (
    <button
      ...
      title={title}
      aria-label={title}
      className={`inline-flex items-center justify-center rounded-lg transition ${ACTION_BUTTON_SIZES[size]} ${toneClasses} ${className}`}
    >
...
export function EmptyState({ title, description }: { title: string; description?: string }) {
  return (
    <div className="text-center py-14 text-slate-500">
      <div className="mx-auto mb-3 h-12 w-12 rounded-full bg-brand-50 text-brand-400 flex items-center justify-center text-xl">✦</div>
