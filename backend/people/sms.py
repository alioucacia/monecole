"""Envoi de SMS — simulation par défaut (journalisée), ou envoi réel via Twilio si configuré
(voir `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_FROM_NUMBER` dans les settings et
.env.example). Interchangeable pour un autre fournisseur plus tard : il suffit de remplacer
`_envoyer_via_twilio` par l'appel à sa propre API, sans toucher au code appelant (`send_sms` est
le seul point d'entrée utilisé partout ailleurs dans le projet).
"""
import logging
